...
col_names = c("Year", "California", "Oregon", "Washington", "Total Landings", "Est. Total Mortality")

table_format(x = out,
             caption = "Removals (mt) by state, the summed total landings across states (mt), and the estimated total mortality (mt) summed by year. Oregon and Washington were combined into a single fleet in the model",
             label = "allcatches",
             digits = 0,
             align = 'l',
             col_names = col_names)
...
tab = read.csv(file.path(data_loc, "catches", "or", "ca_historical_caught_in_or_wa.csv"))

col_names = c("Year", "Oregon (mt)", "Washington (mt)")

table_format(x = tab,
             caption = "Historical removals in metric tons that were caught in Oregon or Washington waters and landed in California that were excluded from the California historical catch reconstruction. These landings were added to the Oregon and Washington historical catch reconstructions",
             label = "or-wa-add-catch",
             align = 'l',
             custom_width = TRUE,
             col_to_adjust = 1:3,
             width = c("2cm", "2.5cm", "2.5cm"),
             digits = 0,
             col_names = col_names)
```
