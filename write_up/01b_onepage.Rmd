...
\item The estimated spawning biomass at the beginning of 2021 was `r ssb_2021` mt ($\sim$95 percent asymptotic intervals: `r ssb_2021_low` to `r ssb_2021_hi` mt), which when compared to unfished spawning biomass (`r ssb0` mt) equates to a relative stock status level of `r round(100*depl_2021,0)` percent ($\sim$95 percent asymptotic intervals: `r round(100*depl_2021_low,0)` to `r round(100*depl_2021_hi,0)` percent).  However, the estimate scale of the stock ($SB_0$) from this assessment, `r ssb0` mt, is lower than the value estimated in the 2011 assessment of 469,866 mt but well within the 2011 $\sim$95 percent asymptotic interval (182,741 - 756,991 mt).

\item  Fishing intensity (1 - SPR) over the past decade has been well below the target $SPR_{30\%}$, ranging between `r min(spr_recent_10)` and `r max(spr_recent_10)`. The estimated target spawning biomass based on the 25 percent management target is `r format(ssb_targ, big.mark = ',')`. Sustainable total yield, landings plus discards, using an $SPR_{30\%}$ is estimated at `r format(model$derived_quants[model$derived_quants$Label == 'Dead_Catch_SPR', 'Value'], digits = 0, scientific = FALSE, big.mark = ",")` mt.   
