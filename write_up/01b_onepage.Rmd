...
\item This assessment for `r spp` incorporates a wide range of data sources: landings data and discard estimates; survey indices of abundance, length- or age-composition data for each fishery or survey (with conditional age-at-length data used for the survey); information on weight-at-length, maturity-at-length, and fecundity-at-length; information on natural mortality and the steepness of the Beverton-Holt stock-recruitment relationship; and estimates of ageing error.

\item The longest source of fishery-independent information off the U.S. west coast arises from the \gls{s-wcgbt} that has been conducted annual from 2003 - 2019. The data from the length and age data from this survey were highly influential in the model estimates of stock size and status. Length-weight parameters were fixed for both sexes at values estimated external to the model using these data.   
...
\item  Fishing intensity (1 - SPR) over the past decade has been well below the target (1 - $SPR_{30\%}$), ranging between `r min(spr_recent_10)` and `r max(spr_recent_10)`. The estimate target spawning biomass based on 25 percent management target is `r format(ssb_targ, big.mark = ',')`. Sustainable total yield, landings plus discards, using an $SPR_{30\%}$ is estimate at `r format(model$derived_quants[model$derived_quants$Label == 'Dead_Catch_SPR', 'Value'], digits = 0, scientific = FALSE, big.mark = ",")` mt.   
