...
for (a in 5:length(all)){
	if(file.exists(file.path(wd, all[a], "Report.sso"))){
		model  = SS_output(file.path(wd, all[a]), printstats = FALSE, 
			verbose = FALSE, covar = FALSE)		
		R0 = model$parameters[model$parameters$Label == 'SR_LN(R0)', 'Value']
		M = round(model$parameters[model$parameters$Label == 'NatM_p_1_Fem_GP_1', 'Value'],3)
		male_M = round(M * exp(model$parameters[model$parameters$Label == 'NatM_p_1_Mal_GP_1', 'Value']),3)
		...
		out = rbind(out, c(basename(all[a]), R0, M, male_M, AFSC_Q, TRI_Q, NWFSC_Q, WCGBT_Q))
	}	
}

write.table(out, file = file.path(wd, "R0_M_Q_values_4.23.txt"))